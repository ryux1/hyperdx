...
<!-- hyperdx-release-notes version=2.39.0 inputs=262776ebff85 -->

**Required dashboard filters and span metrics**

Dashboard filters can now be configured as required rather than optional, so a
dashboard that is only meaningful once it has been scoped can say so. The
bundled OpenTelemetry collector also ships with the spanmetrics connector
compiled in, so you can derive call-count and duration metrics from the spans it
already receives — that matters most if you ingest Datadog Agent traces through
the existing `datadogreceiver`, where until now there was no way to get RED
metrics out of that trace data once it had landed. Nothing changes there unless
you ask for it: the connector only runs when you wire it into your own pipeline
config. The Help menu's "What's new" sparkle now appears only when there is
genuinely a new release to read.
...
- `@hyperdx/common-utils` 0.28.1 → 0.28.2 — [changelog](https://github.com/hyperdxio/hyperdx/blob/main/packages/common-utils/CHANGELOG.md#0282)
